...
public sealed class CustomerService(IDbContextFactory<BoutiqueDbContext> factory) : ICustomerService
{
    ...
    {
        await using var db = await factory.CreateDbContextAsync(cancellationToken);
        var sales = await db.Sales.AsNoTracking().Where(s => s.CustomerId == customerId)
        ...
        var payments = await (from p in db.CreditPayments
                              join c in db.CustomerCredits on p.CustomerCreditId equals c.Id
                              join s in db.Sales on c.SaleId equals s.Id
                              where c.CustomerId == customerId
                              orderby p.CreatedAt descending
                              select new CustomerHistoryPayment(p.Number, p.CreatedAt, p.AmountXof, p.Mode, s.Number)).Take(100).ToListAsync(cancellationToken);
