...
{
    public async Task<IReadOnlyList<DocumentListItem>> ListAsync(CancellationToken cancellationToken=default){await using var db=await factory.CreateDbContextAsync(cancellationToken);var rows=await db.DocumentSnapshots.AsNoTracking().Select(x=>new DocumentListItem(x.Id,x.Number,x.Type,x.CreatedAt,x.PrintCount)).ToListAsync(cancellationToken);return rows.OrderByDescending(x=>x.IssuedAt).Take(500).ToArray();}
